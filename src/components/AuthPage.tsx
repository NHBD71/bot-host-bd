  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === "auth/invalid-credential") {
        errMsg = "ভুল ইমেইল অথবা পাসওয়ার্ড দেওয়া হয়েছে! দয়া করে সঠিক ইমেইল ও পাসওয়ার্ড দিয়ে আবার চেষ্টা করুন অথবা আপনার অ্যাকাউন্ট না থাকলে উপরে 'Register (সাইন আপ)' ট্যাবে গিয়ে নতুন অ্যাকাউন্ট খুলুন। (Incorrect email or password. If you don't have an account, please switch to the Register tab above!)";
      } else if (err.code === "auth/wrong-password") {
        errMsg = "ভুল পাসওয়ার্ড দেওয়া হয়েছে! দয়া করে সঠিক পাসওয়ার্ড দিন অথবা 'Forgot password?' এ ক্লিক করে রিসেট করুন। (Incorrect password.)";
      } else if (err.code === "auth/user-not-found") {
        errMsg = "এই ইমেইল দিয়ে কোনো অ্যাকাউন্ট খুঁজে পাওয়া যায়নি! দয়া করে সঠিক ইমেইল দিন বা ওপরে 'Register (সাইন আপ)' ট্যাবে গিয়ে অ্যাকাউন্ট তৈরি করুন। (User not found.)";
      } else if (err.code === "auth/email-already-in-use") {
        errMsg = "এই ইমেইলটি ইতিমধ্যে অন্য একটি অ্যাকাউন্টে ব্যবহার করা হচ্ছে! দয়া করে অন্য ইমেইল ব্যবহার করুন অথবা ওপরে 'Sign In (লগইন)' ট্যাবে গিয়ে লগইন করুন। (Email already registered.)";
      } else if (err.code === "auth/weak-password") {
        errMsg = "পাসওয়ার্ডটি অত্যন্ত দুর্বল! দয়া করে কমপক্ষে ৬ অক্ষরের একটি শক্তিশালী পাসওয়ার্ড দিন। (Password must be at least 6 characters.)";
      } else if (err.code === "auth/invalid-email") {
        errMsg = "অনুপযুক্ত বা ভুল ইমেইল অ্যাড্রেস দেওয়া হয়েছে। দয়া করে সঠিক ইমেইল লিখুন। (Invalid email format.)";
    ...
    <div className="min-h-screen bg-[#0b0f19] flex flex-col justify-center items-center p-4 sm:p-6 relative">
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <span className="font-display font-bold text-2xl tracking-tight text-white text-center">
            {mode === "login" && "Welcome Back / লগইন করুন"}
            {mode === "register" && "Create Account / সাইন আপ"}
            {mode === "forgot_password" && "Reset Password / পাসওয়ার্ড রিসেট"}
            {mode === "verification_sent" && "Verify Email / ইমেইল যাচাই"}
          </span>
          <p className="text-xs text-gray-400 mt-2 text-center max-w-xs">
            {mode === "login" && "ইমেইল ও পাসওয়ার্ড দিয়ে আপনার অ্যাকাউন্টে লগইন করুন (Enter details to monitor python servers)"}
            {mode === "register" && "নতুন অ্যাকাউন্ট খুলতে নিচের ফর্মটি পূরণ করুন (Deploy bots instantly on isolated host hardware)"}
            {mode === "forgot_password" && "পাসওয়ার্ড ভুলে গেলে নিচে আপনার ইমেইল দিন (Receive security recovery verification links)"}
            {mode === "verification_sent" && "Authentication registered successfully (ইমেইল যাচাইকরণ লিংক পাঠানো হয়েছে)"}
          </p>
        </div>

        {/* Navigation Tab Selector for Manual Sign In & Register */}
        {(mode === "login" || mode === "register") && (
          <div className="flex bg-[#070c14]/80 p-1 rounded-xl mb-6 border border-white/5 relative z-20">
            <button
              type="button"
              onClick={() => {
                setError(null);
              ...
              className={`flex-1 py-3 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all duration-300 font-display cursor-pointer flex justify-center items-center ${
                mode === "login"
                  ? "bg-emerald-500 text-black shadow-[0_2px_8px_rgba(16,185,129,0.25)] font-bold"
        ...
        {mode !== "verification_sent" ? (
          <form onSubmit={handleEmailAuth} className="space-y-4">
            {mode === "register" && (
              <div>
                <label className="block text-xs text-gray-400 font-mono mb-2 uppercase tracking-wide">Your Full Name / আপনার নাম</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    placeholder="Enter full name (আপনার নাম লিখুন)"
            ...
            <div>
              <label className="block text-xs text-gray-400 font-mono mb-2 uppercase tracking-wide">Email Address / ইমেইল ঠিকানা</label>
              <div className="relative">
                ...
                <input 
                  ...
                  placeholder="correct_email@gmail.com"
            ...
            {mode !== "forgot_password" && (
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label className="text-xs text-gray-400 font-mono uppercase tracking-wide">Security Password / পাসওয়ার্ড</label>
                  {mode === "login" && (
                    <button 
                      type="button" 
                    ...
                    >
                      Forgot password? (পাসওয়ার্ড মনে নেই?)
                    </button>
                  )}
                </div>
                <div className="relative">
            ...
            <button 
              ...
              disabled={loading}
              id="auth-submit-btn"
              className="w-full py-3 bg-emerald-500 hover:bg-emerald-400 disabled:bg-emerald-800 text-black font-display font-semibold rounded-xl text-xs tracking-wider transition-all duration-300 cursor-pointer shadow-[0_4px_12px_rgba(16,185,129,0.2)] disabled:cursor-not-allowed mt-6 border-none"
            >
              {loading ? "AUTHENTICATING SERVER LOGS..." : (
                mode === "login" ? "Sign In (লগইন করুন)" : 
                mode === "register" ? "Register / Create Account (নতুন অ্যাকাউন্ট খুলুন)" : "Send Recovery Mail / লিঙ্ক পাঠান"
              )}
            </button>
          </form>
        ) : (
          <div className="space-y-6 text-center">
            ...
            <p className="text-[11px] text-emerald-400 leading-relaxed max-w-sm mx-auto">
              (আমরা আপনার ইমেইল এড্রেসে একটি লিঙ্ক পাঠিয়েছি। অনুগ্রহ করে আপনার ইমেইল চেক করুন এবং লিঙ্কে ক্লিক করে ভেরিফাই করুন।)
            </p>
            <button 
            ...
            <div className="mt-8 text-center">
              {mode === "login" && (
                <p className="text-xs text-gray-400">
                  ...
                  >
                    Create client account / নতুন অ্যাকাউন্ট খুলুন
                  </button>
                </p>
              )}
