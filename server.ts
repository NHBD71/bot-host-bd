...
// Render Keep-Alive / Self-Ping Service to prevent free-tier dyno sleep
function startKeepAlivePing() {
  const externalUrl = process.env.RENDER_EXTERNAL_URL || process.env.PUBLIC_URL;
  if (!externalUrl) {
    console.log("[Keep-Alive] RENDER_EXTERNAL_URL or PUBLIC_URL environment variables are not set. Self-ping keep-alive stands by.");
    return;
  }

  const pingUrl = externalUrl.endsWith("/") ? `${externalUrl}api/health` : `${externalUrl}/api/health`;
  console.log(`[Keep-Alive] Initializing self-ping keep-alive service. Target URL: ${pingUrl}`);
  ...
  // Run ping every 5 minutes (300,000 milliseconds) to reliably prevent Render's 15-minute inactivity sleep
  setInterval(() => {
    pingTarget(pingUrl);
  }, 300000);
}

async function pingTarget(url: string) {
  try {
    const response = await fetch(url);
    console.log(`[Keep-Alive] Self-ping successfully triggered at ${new Date().toISOString()}. Status: ${response.status} (${response.statusText})`);
  } catch (err: any) {
    console.error(`[Keep-Alive] Self-ping failed at ${new Date().toISOString()}:`, err.message || err);
  }
}
...
async function startServer() {
  ...
  // Start self-pinging loop to keep the server awake on Render or Railway
  startKeepAlivePing();
